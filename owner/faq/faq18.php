...
<?php 
    if(isset($_POST['save'])) {
        ...
        if($result) {
            ...
            $sql = "UPDATE `client_data` SET `ques18`='$_POST[question]',`ans18`='$name', `date18`=NOW() WHERE `ques18` IS NULL ORDER BY id DESC LIMIT 1";
            $res = mysqli_query($con, $sql);
            ...
            if($res) {
                echo"
                    <script>
                        ...
                        window.location.href = 'faq19.php';
                    </script>
                ";
            }
        }    
    ...
    if(isset($_POST['update'])) {
        if(isset($_GET['id']) && $_GET['topic']) {
            ...
            $query = "SELECT * FROM `client_data` WHERE `ques18` IS NULL AND `id`='$id'";
            $result = mysqli_query($con, $query);
            ...
            if($result) {
                ...
                $sql = "UPDATE `client_data` SET `ques18`='$_POST[question]',`ans18`='$name', `date18`=NOW() WHERE `topic`='$topic'";
                $res = mysqli_query($con, $sql);
