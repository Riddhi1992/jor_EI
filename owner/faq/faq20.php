            ...
            <div id="" class="">
                ...
                <div class="selectmenu form-floating mb-3">
                    <select class="form-select" aria-label="Floating label select example" id="selection" name="question" onchange="selectOption()" required>
                        <option value="Selection" selected>Select Twentieth question</option>
                        <?php 
                            if(isset($_GET['option']) || $_GET['id']){
                                ...
                                $sql2 = "SELECT * FROM `option_Selection` WHERE `purchase_lease` = '$option' AND NOT `question`='$ques1' AND NOT `question`='$ques2' AND NOT `question`='$ques3' AND NOT `question`='$ques4' AND NOT `question`='$ques5' AND NOT `question`='$ques6' AND NOT `question`='$ques7' AND NOT `question`='$ques8' AND NOT `question`='$ques9' AND NOT `question`='$ques10' AND NOT `question`='$ques11' AND NOT `question`='$ques12' AND NOT `question`='$ques13' AND NOT `question`='$ques14' AND NOT `question`='$ques15' AND NOT `question`='$ques16' AND NOT `question`='$ques17' AND NOT `question`='$ques18' AND NOT `question`='$ques19' AND NOT `question`='$q1' AND NOT `question`='$q2' AND NOT `question`='$q3' AND NOT `question`='$q4' AND NOT `question`='$q5' AND NOT `question`='$q6' AND NOT `question`='$q7' AND NOT `question`='$q8' AND NOT `question`='$q9' AND NOT `question`='$q10' AND NOT `question`='$q11' AND NOT `question`='$q12' AND NOT `question`='$q13' AND NOT `question`='$q14' AND NOT `question`='$q15' AND NOT `question`='$q16' AND NOT `question`='$q17' AND NOT `question`='$q18' AND NOT `question`='$q19'";
...
<?php 
    ...
    if(isset($_POST['save']) && $_GET['property_type'] && $_GET['option']) {
        ...
        $option = $_GET['option'];
        $query = "SELECT * FROM `client_data` WHERE `ques20` IS NULL AND `ans20` IS NULL";
        $result = mysqli_query($con, $query);
        ...
        if($result) {
            ...
            if($key_res) {
                $sql = "UPDATE `client_data` SET `ques20`='$_POST[question]',`ans20`='$name',`topic_ques20`=CONCAT('$option', ' - ', '$keyword'), `date20`=NOW() WHERE `ques20` IS NULL ORDER BY id DESC LIMIT 1";
                $res = mysqli_query($con, $sql);
            
                if($res) {
                    echo"
                        <script>
                            alert('Twentieth Question updated successfully!');
                        </script>
                    ";
                }
            }
    ...
    if(isset($_POST['update'])) {
        if(isset($_GET['id']) && $_GET['address'] && $_GET['option']) {
            ...
            if($result) {
                ...
                if($key_res) {
                    $sql = "UPDATE `client_data` SET `ques20`='$_POST[question]',`ans20`='$name',`topic_ques20`=CONCAT('$opt', ' - ', '$keyword'), `date20`=NOW() WHERE `id`='$id'";
                    $res = mysqli_query($con, $sql);
                
                    if($res) {
                        echo"
                            <script>
                                alert('Twentieth Question added successfully!');
