                ...
                <div id="" class="">
                    ...
                    <div class="selectmenu form-floating mb-3">
                        <select class="form-select" aria-label="Floating label select example" id="selection" name="question" onchange="selectOption()" required>
                            <option value="Selection" selected>Select Tenth question</option>
                            <?php 
                                if(isset($_GET['option']) || $_GET['id']){
                                    ...
                                    $sql2 = "SELECT * FROM `option_Selection` WHERE `purchase_lease` = '$option' AND NOT `question`='$ques1' AND NOT `question`='$ques2' AND NOT `question`='$ques3' AND NOT `question`='$ques4' AND NOT `question`='$ques5' AND NOT `question`='$ques6' AND NOT `question`='$ques7' AND NOT `question`='$ques8' AND NOT `question`='$ques9' AND NOT `question`='$q1' AND NOT `question`='$q2' AND NOT `question`='$q3' AND NOT `question`='$q4' AND NOT `question`='$q5' AND NOT `question`='$q6' AND NOT `question`='$q7' AND NOT `question`='$q8' AND NOT `question`='$q9'";
...
<?php 
    ...
    if(isset($_POST['save']) && $_GET['property_type'] && $_GET['option']) {
        ...
        if($result) {
            ...
            if($key_res) {
                $sql = "UPDATE `client_data` SET `ques10`='$_POST[question]',`ans10`='$name',`topic_ques10`=CONCAT('$option', ' - ', '$keyword'), `date10`=NOW() WHERE `ques10` IS NULL ORDER BY id DESC LIMIT 1";
                ...
                if($res) {
                    echo"
                        <script>
                            ...
                            window.location.href = 'faq11.php?property_type=$property_type&option=$option';
                        </script>
                    ";
                }
            }
    ...
    if(isset($_POST['update'])) {
        if(isset($_GET['id']) && $_GET['address'] && $_GET['option']) {
            ...
            if($result) {
                ...
                if($key_res) {
                    $sql = "UPDATE `client_data` SET `ques10`='$_POST[question]',`ans10`='$name',`topic_ques10`=CONCAT('$opt', ' - ', '$keyword'), `date10`=NOW() WHERE `id`='$id'";
                    $res = mysqli_query($con, $sql);
