...
<?php 
    if(isset($_POST['save'])) {
        ...
        if($result) {
            ...
            $sql = "UPDATE `client_data` SET `ques14`='$_POST[question]',`ans14`='$name', `date14`=NOW() WHERE `ques14` IS NULL ORDER BY id DESC LIMIT 1";
            $res = mysqli_query($con, $sql);
            ...
            if($res) {
                echo"
                    <script>
                        ...
                        window.location.href = 'faq15.php';
                    </script>
                ";
            }
        }    
    ...
    if(isset($_POST['update'])) {
        if(isset($_GET['id']) && $_GET['topic']) {
            ...
            $query = "SELECT * FROM `client_data` WHERE `ques14` IS NULL AND `id`='$id'";
            $result = mysqli_query($con, $query);
            ...
            if($result) {
                ...
                $sql = "UPDATE `client_data` SET `ques14`='$_POST[question]',`ans14`='$name', `date14`=NOW() WHERE `topic`='$topic'";
                $res = mysqli_query($con, $sql);
