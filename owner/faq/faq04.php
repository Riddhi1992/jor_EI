...
<?php 
    ...
    if(isset($_POST['save']) && $_GET['property_type'] && $_GET['option']) {
        ...
        if($result) {
            ...
            if($key_res) {
                $sql = "UPDATE `client_data` SET `ques4`='$_POST[question]',`ans4`='$name',`topic_ques04`=CONCAT('$option', ' - ', '$keyword'), `date4`=NOW() WHERE `ques4` IS NULL ORDER BY id DESC LIMIT 1";
                ...
                if($res) {
                    echo"
                        <script>
                            ...
                            window.location.href = 'faq05.php?property_type=$property_type&option=$option';
                        </script>
                    ";
                }
            }
    ...
    if(isset($_POST['update'])) {
        if(isset($_GET['id']) && $_GET['address'] && $_GET['option']) {
            ...
            if($result) {
                ...
                if($key_res) {
                    $sql = "UPDATE `client_data` SET `ques4`='$_POST[question]',`ans4`='$name',`topic_ques04`=CONCAT('$opt', ' - ', '$keyword'), `date4`=NOW() WHERE `id`='$id'";
                    $res = mysqli_query($con, $sql);
