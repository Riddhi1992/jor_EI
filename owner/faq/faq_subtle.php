        ...
        <div id="faqForm" class='shadow mt-5'>
            ...
            <form method="POST" class="" action="" enctype="multipart/form-data">
                <div id='first'>
                    ...
                    <div class='mb-3'>
                        <label for='formFile' class='form-label'>Insert your Subtle Video (Note: In this video, you will be trying to listen.)</label>
...
<?php 
    ...
    if(isset($_POST['save']) && $_GET['property_type'] && $_GET['option']) {
        ...
        if($result) {
            ...
            $sql = "UPDATE `client_data` SET `subtle`='$name' WHERE `ques1` IS NULL ORDER BY id DESC LIMIT 1";
            $res = mysqli_query($con, $sql);

            if($res) {
                echo"
                    <script>
                        window.location.href = 'faq_generic.php?property_type=$property_type&option=$option';
                    </script>
                ";
            }
        }        
    ...
    if(isset($_POST['update'])) {
        if(isset($_GET['id']) && $_GET['address'] && $_GET['option']) {
            ...
            if($result) {
                ...
                $sql = "UPDATE `client_data` SET `subtle`='$name' WHERE `id`='$id'";
                $res = mysqli_query($con, $sql);
