                ...
                <div id="" class="">
                    ...
                    <div class="selectmenu form-floating mb-3">
                        <select class="form-select" aria-label="Floating label select example" id="selection" name="question" onchange="selectOption()" required>
                            <option value="Selection" selected>Select Eight question</option>
                            <?php 
                                if(isset($_GET['option']) || $_GET['id']){
                                    ...
                                    $sql2 = "SELECT * FROM `option_Selection` WHERE `purchase_lease` = '$option' AND NOT `question`='$ques1' AND NOT `question`='$ques2' AND NOT `question`='$ques3' AND NOT `question`='$ques4' AND NOT `question`='$ques5' AND NOT `question`='$ques6' AND NOT `question`='$ques7' AND NOT `question`='$q1' AND NOT `question`='$q2' AND NOT `question`='$q3' AND NOT `question`='$q4' AND NOT `question`='$q5' AND NOT `question`='$q6' AND NOT `question`='$q7'";
...
<?php 
    if(isset($_POST['save']) && $_GET['property_type'] && $_GET['option']) {
        ...
        $option = $_GET['option'];
        $query = "SELECT * FROM `client_data` WHERE `ques8` IS NULL AND `ans8` IS NULL";
        $result = mysqli_query($con, $query);
        ...
        if($result) {
            ...
            if($key_res) {
                $sql = "UPDATE `client_data` SET `ques8`='$_POST[question]',`ans8`='$name',`topic_ques08`=CONCAT('$option', ' - ', '$keyword'), `date8`=NOW() WHERE `ques8` IS NULL ORDER BY id DESC LIMIT 1";
                $res = mysqli_query($con, $sql);
            
                if($res) {
                    echo"
                        <script>
                            alert('Eight Question updated successfully!');
                            window.location.href = 'faq09.php?property_type=$property_type&option=$option';
                        </script>
                    ";
                }
            }
    ...
    if(isset($_POST['update'])) {
        if(isset($_GET['id']) && $_GET['address'] && $_GET['option']) {
            ...
            if($result) {
                ...
                if($key_res) {
                    $sql = "UPDATE `client_data` SET `ques8`='$_POST[question]',`ans8`='$name',`topic_ques08`=CONCAT('$opt', ' - ', '$keyword'), `date8`=NOW() WHERE `id`='$id'";
                    $res = mysqli_query($con, $sql);
                
                    if($res) {
                        echo"
                            <script>
                                alert('Eight Question added successfully!');
