                ...
                <div id="" class="">
                    ...
                    <div class="selectmenu form-floating mb-3">
                        <select class="form-select" aria-label="Floating label select example" id="selection" name="question" onchange="selectOption()" required>
                            <option value="Selection" selected>Select fifth question</option>
                            <?php 
                                if(isset($_GET['option']) || $_GET['id']){
                                    ...
                                    $sql2 = "SELECT * FROM `option_Selection` WHERE `purchase_lease` = '$option' AND NOT `question`='$ques1' AND NOT `question`='$ques2' AND NOT `question`='$ques3' AND NOT `question`='$ques4' AND NOT `question`='$q1' AND NOT `question`='$q2' AND NOT `question`='$q3' AND NOT `question`='$q4'";
...
<?php 
    ...
    if(isset($_POST['save']) && $_GET['property_type'] && $_GET['option']) {
        ...
        $option = $_GET['option'];
        $query = "SELECT * FROM `client_data` WHERE `ques5` IS NULL AND `ans5` IS NULL";
        $result = mysqli_query($con, $query);
        ...
        if($result) {
            ...
            if($key_res) {
                $sql = "UPDATE `client_data` SET `ques5`='$_POST[question]',`ans5`='$name', `topic_ques05`=CONCAT('$option', ' - ', '$keyword'), `date5`=NOW() WHERE `ques5` IS NULL ORDER BY id DESC LIMIT 1";
                $res = mysqli_query($con, $sql);
            
                if($res) {
                    echo"
                        <script>
                            alert('Fifth Question updated successfully!');
                            window.location.href = 'faq06.php?property_type=$property_type&option=$option';
                        </script>
                    ";
                }
            }
    ...
    if(isset($_POST['update'])) {
        if(isset($_GET['id']) && $_GET['address'] && $_GET['option']) {
            ...
            if($result) {
                ...
                if($key_res) {
                    $sql = "UPDATE `client_data` SET `ques5`='$_POST[question]',`ans5`='$name',`topic_ques05`=CONCAT('$opt', ' - ', '$keyword'), `date5`=NOW() WHERE `id`='$id'";
                    $res = mysqli_query($con, $sql);
                
                    if($res) {
                        echo"
                            <script>
                                alert('Fifth Question added successfully!');
